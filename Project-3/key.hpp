...
typedef uint key_type; 	// 4 bytes/32 bits
#define C_IS_SET
...
typedef unsigned long long key_type; 	// 8 bytes/64 bits
#define C_IS_SET
...
public:
    ...
    //return a value representing this key
    std::string str();
};
...
inline std::string Key::str() {
	char value[C];

	for (int i = 0; i < C; i++) {
		value[i] = ALPHABET[m_digit[i]];		
	}
